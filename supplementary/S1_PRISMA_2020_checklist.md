...
**Completed:** 21 Aug 2026; revised 10 Sep 2026 (Items 6, 7, 27).
**Supplementary ID:** S1

This checklist maps each of the 27 PRISMA 2020 items to the section, table, figure, or appendix subsection of the manuscript where it is reported. All funnel counts are traceable record-by-record through the pipeline files (Appendix A, `sec:appendix:corpus_pipeline`) and are re-derived by the two `verify_*.py` scripts in the `mdpi_slr_engine` repository (both pass, 2026-09-10).
...
| Section | # | Checklist item | Where the item is reported |
|---|---|---|---|
| **Eligibility criteria** | 5 | Specify the inclusion and exclusion criteria for the review and how studies were grouped for the syntheses. | §3.1 Search Strategy and Corpus (`sec:methodology:search`); §3 Materials and Methods (`sec:methodology`); **Appendix A.3** (Inclusion and Exclusion Criteria); the chronological search-execution log table in Appendix D (`tab:search_log`) |
| **Information sources** | 6 | Specify all databases, registers, websites, organisations, reference lists, and other sources searched or consulted to identify studies. Specify the date when each source was last searched or consulted. | §3.1 (`sec:methodology:search`); **Appendix A.1** (Undermind-assisted retrieval for pre-validated Groups G1–G6, 85.4% of the final corpus; six deep searches run 2026-04-14, per-group identification funnel in Table `tab:undermind_funnel`: 377 exported → 343 after cross-group dedup → 105 in final list) and **Appendix A.2** (Citation Snowballing) with **Appendix D.2** (snowball search date 2026-04-21); sources: Undermind (undermind.ai), Semantic Scholar Academic Graph API, Scopus, ACL Anthology, Web of Science (post-hoc enrichment role only) (see `PRISMA_summary_2026-04-21.md`). A separate dedicated search for competing secondary studies (systematic reviews / surveys) was run on **2026-09-10** across Web of Science and Scopus (Table `tab:related_surveys`; strings in Appendix C `app:db_queries:related`; timeline row in Appendix D.1) |
| **Search strategy** | 7 | Present the full search strategies for all databases, registers, and websites, including any filters and limits used. | **Appendix C** (Manual Database Search Queries — full Scopus and WoS query strings for the primary corpus, plus the two related-reviews query strings, `app:db_queries:related`); **Appendix D.3** (Forward citation snowballing) |
| **Selection process** | 8 | Specify the methods used to decide whether a study met the inclusion criteria of the review, including how many reviewers screened each record and each report retrieved, whether they worked independently, and, if applicable, details of automation tools used. | §3.1 (`sec:methodology:search`); **Appendix A.4** (Screening Procedure — three-layer title screening: year filter, keyword scoring, manual triage); **Appendix A.7** (Abstract-Level Review). Single-reviewer screening (the author), with an AI-assisted classifier used only in an advisory/triage-consistency role; all inclusion/exclusion decisions were author-made. No inter-rater reliability statistic (e.g., Cohen's κ) is reported: PRISMA 2020 Item 8 requires disclosing reviewer count, not dual independent screening, and a coefficient computed against a non-independent automated classifier would not represent valid independent human agreement (§3.1, `sec:methodology:screening_reviewers`) |
...
| | 13f | Describe any sensitivity analyses conducted to assess robustness of the synthesised results. | Funnel cross-validation and per-stage reconciliation (recomputed by the `mdpi_slr_engine` scripts `src/slr_engine/verify/verify_prisma_counts.py` / `src/slr_engine/verify/verify_quality_appraisal.py`, run outputs in `mdpi_paper_slr` under `snowball_output/verification_runs/`); §3.1 |
...
| Section | # | Checklist item | Where the item is reported |
|---|---|---|---|
| **Study selection** | 16a | Describe the results of the search and selection process, from the number of records identified in the search to the number of studies included in the review, ideally using a flow diagram. | **Figure 5 (PRISMA flow diagram, `fig:prisma`)** — 1,150 raw → 972 dedup → 502 merged → 464 enriched → 552 abstract-reviewed → 387 full-text queue → 224 extraction → 123 final (120 distinct). Appendix D search-log table (`tab:search_log`); `PRISMA_summary_2026-04-21.md` |
| | 16b | Cite studies that might appear to have met the inclusion criteria, but which were excluded, and explain why they were excluded. | **Appendix A.4** (Table 2, title-screening exclusions: 731 by Layer-2 keyword/LLM-only-match scoring, 60 by Layer-3 manual triage, 791 total; per-record decisions logged in `log_screening_2026-04-21.json`); abstract-review SKIP 338 with reasons (Appendix A.7) |
...
| **Risk of bias in studies** | 18 | Present assessments of risk of bias for each included study. | **§3.3 Quality Assessment** (`sec:methodology:quality`) + **Fig. 6** (`fig:quality_bands`) — reporting-completeness score distribution across all 123 records (mean 6.04/10, SD=2.00, median 6): 13 records (10.6%) high band (9–10), 61 (49.6%) upper-mid (6–8), 47 (38.2%) lower-mid (3–5), 2 (1.6%) low band (0–2); Q5 (contribution clarity) scored separately over its populated subpopulation (n=16), excluded from this total. Full record-by-record scores in `figures/quality_appraisal_scored.csv` |
...
| | 20d | Present results of all sensitivity analyses conducted to assess the robustness of the synthesised results. | Funnel reconciliation and per-stage counts (recomputed by the `mdpi_slr_engine` scripts `src/slr_engine/verify/verify_prisma_counts.py` / `src/slr_engine/verify/verify_quality_appraisal.py`, outputs in `mdpi_paper_slr` under `snowball_output/verification_runs/`) |
...
| **Availability of data, code, and other materials** | 27 | Report which of the following are publicly available and where they can be found: template data collection forms; data extracted from included studies; data used for all analyses; analytic code; any other materials used in the review. | Held in two publicly available repositories (see Data Availability statement): `mdpi_paper_slr` (this manuscript, curated corpus `papers_repo/G0–G6`, PRISMA package, and the per-record pipeline artefacts) at <https://github.com/vanjaluk8/mdpi_paper_slr> and `mdpi_slr_engine` (code-only snowballing/retrieval/screening/extraction pipeline source) at <https://github.com/vanjaluk8/mdpi_slr_engine>. Version-controlled per-record artefacts in `mdpi_paper_slr`: `13_final_reading_list_2026-05-12.csv` (final reading list), `pipeline_unified.csv` (per-stage membership of every record), `log_retrieval_2026-04-21.json` and `log_screening_2026-04-21.json` (retrieval log; per-record title-screening decisions), `S7b_abstract_reviewed_final.csv` (per-record abstract-review disposition), `11_data_extraction_2026-05-12.csv` (224-row extraction spreadsheet), `14_final_curation_reasons.csv` (per-record eligibility-stage exclusion codings), `PRISMA_summary_2026-04-21.md`. Analytic/verification code in `mdpi_slr_engine`: `src/slr_engine/verify/verify_prisma_counts.py`, `src/slr_engine/verify/verify_quality_appraisal.py` (run outputs in `mdpi_paper_slr` under `snowball_output/verification_runs/`). Per-study quality-appraisal scores: `quality_appraisal_scored.csv` (Supplementary S2). |
...
| Records retrieved (raw) | 1,150 | `log_retrieval_2026-04-21.json` (combined backward+forward snowball on G0; per-direction split not reported in-text) |
