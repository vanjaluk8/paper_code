...
This section introduces the three technical foundations on which the
...
literature reviewed in later sections. Readers familiar with these topics
...
\begin{equation}
\mathrm{Adapter}(h) = W_{\mathrm{up}} \; \sigma( W_{\mathrm{down}} \, h ) + h
\label{eq:bottleneck}
\end{equation}
...
entire model to parameter-efficient adapters trained per task.}
  \label{fig:concept_peft_paradigm}
\end{figure}

\begin{figure}[H]
  \centering
  % The three canonical adapter structures (own figure).
  \includegraphics[width=0.8\textwidth]{figures/fig_peft_adapter_types.pdf}
...
probability~\cite{KarpRumor2000}. Gossip protocols are inherently fault-tolerant
...
\begin{figure}[H]
  ...
  \caption{Gossip propagation: an informed peer spreads a piece of state to randomly chosen neighbours each round until the entire network is aware. (Adapter metadata is one example payload discussed in later sections.)}
