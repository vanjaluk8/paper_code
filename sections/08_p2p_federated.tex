...
\begin{figure}[H]
  ...
  \caption{Gossip learning (own figure): each node periodically samples a
    ...
    while maintaining convergence to a stationary point under mild
    graph-connectivity conditions, with guarantees comparable to centralised FL
    \cite{HegeduisGossip2019,Lian2017Decentralized}. This reproduces the
    gossip-propagation diagram from
    Section~\ref{sec:background:p2p} (Figure~\ref{fig:p2p_gossip}) in the
    gossip-learning context.}
  \label{fig:gossip}
\end{figure}
...
{\v{S}}ajina~\cite{Sajina2025} described, to the best of the author's knowledge, one of the first systems for multi-task P2P learning with
...
introduced the Kademlia distributed hash table (DHT) (Figure~\ref{fig:dht_lookup}), which provides $O(\log N)$
...
\begin{figure}[H]
  ...
  \caption{Kademlia DHT $O(\log N)$ lookup (own figure): a querying node routes through
    ...
    The underlying DHT-lookup diagram is introduced in
    Section~\ref{sec:background:p2p} (Figure~\ref{fig:p2p_dht_lookup}).}
...
\noindent The decentralised-learning findings of this section are
