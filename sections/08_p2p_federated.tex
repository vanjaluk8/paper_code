...
FL quality in simulation while eliminating the central aggregator. The comparison is
conditional on compression, not a flat speed advantage: with no compression, FL
converges faster in wall-clock time, while under high compression rates gossip
learning's slower aggregation is offset by its higher communication efficiency,
letting it match or outperform FL overall. The approach scales to large peer
populations without requiring any central infrastructure. Adaptive peer selection:
preferring neighbours with high estimated bandwidth or diverse data distributions
improves convergence speed in heterogeneous networks.
...
model averaging converges to a stationary point under mild conditions on
graph connectivity, with guarantees comparable to centralised FL
\cite{Lian2017Decentralized}, providing the theoretical foundation for applying
...
\subsection{P2P Transformer Inference and the MT-EF (Multi-Task Encoder Fusion) Architecture}
...
{\v{S}}ajina~\cite{Sajina2025} described, within the 123-record corpus, one of the first systems for multi-task P2P learning with
...
Borzunov et al.~\cite{Borzunov2023} introduced Petals (Figure~\ref{fig:petals}), which applies P2P principles to LLM inference
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_petals.pdf}
...
matrices reduces per-round communication by over 90\% compared to exchanging full model
...
Zhang et al.~\cite{ZhangFedPETuning2023} conducted an early systematic study of federated PEFT
...
adaptive quantisation of federated LoRA weights for heterogeneous devices, enabling
...
primitives for performing this coordination without a central node: the Kademlia DHT
introduced in Section~\ref{sec:background:p2p} (Figure~\ref{fig:p2p_dht_lookup},
$O(\log N)$ lookup by XOR-distance~\cite{MaymounkovKademlia2002}) is a candidate for
adapter-level discovery, applied here rather than re-described.
...
Recent work on decentralised LoRA, such as Dec-LoRA by Ghiasvand et al.~\cite{Ghiasvand2025}, demonstrates that parameter-efficient LLM fine‑tuning can be performed without a central server over decentralised communication topologies, motivating similar designs for adapter-based systems. Note that Dec-LoRA addresses decentralised co-training of a single shared adapter via gossip-averaged updates on private per-client data, a distinct problem from the decentralised exchange of independently pre-trained, task-differentiated adapters that this review's systems gap (Section~\ref{sec:synthesis_gap:gaps}) concerns. DeCAF~\cite{SaadatiDeCAF2026} (post-review top-up find, \S\ref{sec:methodology:search}) targets the same co-training setting: it improves decentralised LoRA's convergence rate to match decentralised SGD and resolves gradient-consensus interference between peers via truncated-SVD factorisation of the shared adapter. Like Dec-LoRA, it treats the co-trained adapter itself as the object of decentralisation rather than as an independently pre-trained, task-differentiated module to be exchanged — the systems gap above remains open.

In principle, adapter descriptors could be indexed as key-value entries in a DHT, allowing peers to locate task-specific modules without a central registry. Whether task capability descriptors are sufficiently structured for DHT-based retrieval — and how such an index would perform under realistic churn — are open questions that the existing P2P literature has not examined.
