...
Pfeiffer et al.~\cite{PfeifferModular2023} provided a systematic taxonomy of modular deep learning, classifying
...
contributions. (Pfeiffer et al.\ organise modular architectures across four dimensions of
variation~\cite{PfeifferModular2023}; the three operational families used here: routing, merging,
...
ecosystem to support LoRA, prefix tuning, IA$^3$~\cite{LiuIA3_2022} (Infused Adapter by
Inhibiting and Amplifying Inner Activations), and bottleneck adapters under a single
...
Pfeiffer et al.~\cite{Pfeiffer2021fusion} proposed AdapterFusion (Figure~\ref{fig:adapterfusion}), a two-stage approach to multi-task
...
without any additional training. That demonstration used full vision models
fine-tuned from a \emph{shared} pre-trained initialisation on the \emph{same}
task, so the averaged checkpoints lie in a common low-loss region. Independently
trained LoRA adapters do not meet that precondition: the down-projection $A$ is
randomly initialised per run and the up-projection $B$ is zero-initialised, so
two separately trained adapters share no initialisation point and are not
guaranteed to occupy a common basin. Whether mean-averaging their weight
matrices nonetheless yields a useful composed adapter, letting a peer combine
adapters from different sources without access to any original training data,
is therefore a \emph{plausible but empirically unvalidated} zero-shot strategy,
and is among the open questions this review identifies
