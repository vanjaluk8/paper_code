...
Fedus et al.~\cite{Fedus2022} introduced Switch Transformers (Figure~\ref{fig:switch_transformer}), which replace the feedforward
...
a 7$\times$ pre-training speedup over dense T5 at equal computational cost, scaling
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_switch_transformer.pdf}
...
to minimise cross-node communication. Li et al.~\cite{LiAdaFuse2026} (post-review
top-up find, \S\ref{sec:methodology:search}) located an analogous bottleneck one
level down, in single-node MoE-style adapter routing: per-layer dynamic
LoRA-adapter selection fragments execution into many small sequential CUDA
kernel launches, more than doubling decoding latency despite negligible added
compute. Their system, AdaFuse, makes one routing decision per token covering
all adapter layers, then fuses the selected adapters into the backbone with a
single kernel launch, recovering most of the lost throughput.
...
MOELoRA (Liu et al.) \cite{LiuWhenMOE2024} attaches multiple LoRA experts within each
fine-tuned layer and combines their contributions with a task-motivated gate function
driven by task identity rather than per-token input content, producing one fixed
parameter combination per task rather than a per-sample or per-token routing decision.
The remaining methods instead condition routing on the input itself, varying in how
those routing weights are
...
MoELoRA (Luo et al.) \cite{LuoMoELoRA2024} and Mixture of LoRA Experts \cite{WuMixtureLoRA2024}
...
that trains multiple instances of a single chosen PEFT method per layer (e.g.\ a mixture
of bottleneck adapters, or separately a mixture of LoRA matrices, but not both combined)
via stochastic routing during training, merging them into one module at inference for
zero added cost. AdaMix outperforms the corresponding single-instance PEFT method on
GLUE and SuperGLUE at matched parameter budgets, demonstrating that mixing multiple
instances of one PEFT type outperforms a single instance of that type.
