...
adaptation of the citation-snowballing procedure of Wohlin et
al.~\cite{Wohlin2014} (one forward/backward pass on the seed set, not iterated
...
accessed through the Claude Code interface, were also used to draft and
proofread this manuscript. None of these tools made any
inclusion, exclusion, appraisal, or synthesis decision; the author conducted
all such decisions, verified all reported numbers against the underlying
...
Appendix~\ref{app:db_queries:adversarial}). No located artefact provides that
...
field, is therefore reported separately in Table~\ref{tab:discovery_route}. It
confirms that the 105-record figure above is a genuine targeted-retrieval
count and not an artefact of group labelling: only \textbf{14} of the 123
final records were obtained by blind citation snowballing, and the two
supplementary manual passes (the 88-record Scopus/WoS forward-snowball pass and
the 34-record cross-validation top-up) retained none. The route split is given
in Table~\ref{tab:discovery_route}, alongside Table~\ref{tab:groups}.
...
coverage (dedicated related-reviews search, 2026-09-10; \S\ref{sec:methodology:search};
query strings in Appendix~\ref{app:db_queries}).
...
curation of Groups G1--G6; ``direct G0 seed'' are the four seeds themselves
(S-LoRA, AdapterHub, AdapterFusion, Petals); ``citation snowballing'' is the
one-wave forward/backward pass on the G0 seeds. The two remaining routes were
executed (Appendix~\ref{app:search_log}) but retained no records. The \%
...
\midrule
Targeted retrieval (pre-validated G1--G6, Undermind-assisted) & 105 & 85.4 \\
Direct G0 seed & 4 & 3.3 \\
...
\begin{figure}[H]
  ...
  \caption{PRISMA 2020 flow diagram (two-column variant). \emph{Left column ---
           ...
           low-citation preprints moved to a rescue file, later confirmed
           excluded at abstract review), an abstract-review pool of
...
({\v{S}}ajina et al.)---no artifact was ever released; for a further
...
which does not rule out an unreleased or unadvertised one existing. The gap is concentrated among the systems closest to this
