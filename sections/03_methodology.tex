...
thematically focused pre-validated groups (Supplementary Material S4, \S~B); it
...
reproduced verbatim in Supplementary Material S4, \S~B): \textbf{377} records
...
taken further) were excluded. Unlike the screening-stage SKIPs above, this
is a full-text/eligibility-stage exclusion, where PRISMA 2020 Item 16b
requires reasons. The 173 DEFER records were queued for closer full-text
inspection because they were borderline; the author inspected and dropped
them, since each was the same as, or a small increment on, a system
already retained elsewhere in the corpus. The great majority fall into
categories the corpus already represents through included exemplars:
63 single-model PEFT method variants, 32 MoE-LoRA routing or composition
variants, 30 multi-tenant LoRA serving systems, and 25 further federated
PEFT systems, with the remaining 23 concerning P2P or fully decentralised
...
the two query strings are reproduced in Supplementary Material S4, \S~C. The
...
Supplementary Material S4, \S~C.4). No located artefact provides that
...
Supplementary Material S4, \S~C.5 and \S~D,
...
and their motivations are detailed in the appendix. While this criterion alone
would admit any record addressing at least one theme, the corpus is not
undifferentiated: each retained record's relevance position is coded as
\texttt{core} (addressing at least two of the three themes together) or
\texttt{background} (single-theme, retained for foundational context) in the
pipeline's \texttt{corpus} field (Table~\ref{tab:extraction_codebook};
criteria E4/E5, Appendix~\ref{sec:appendix:corpus_pipeline} \S A.3). Of the
123 final records, \textbf{89} (72.4\%) are \texttt{core} and \textbf{34}
(27.6\%) are \texttt{background}. The single-theme admission rule therefore
sets the corpus's outer boundary; the core/background stratification,
already coded for every retained record, is what discriminates within it.
With respect to study type,
...
pre-validated groups (G1--G6, Supplementary Material S4, \S~B), which
...
routes were executed (Supplementary Material S4, \S~D) but retained no records.}
