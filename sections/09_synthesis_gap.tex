...
The preceding sections reviewed five bodies of literature: parameter-efficient
...
gap claims in this section are bounded by the 123-paper corpus assembled
...
This section consolidates the findings of the five review sections
...
\begin{itemize}
    \item A frozen backbone ensures model consistency across all peers and prevents uncontrolled modification, transferring the responsibility for task-specific behaviour entirely to the adapters. This is the founding premise of both foundational PEFT methods in the corpus, bottleneck adapters~\cite{Houlsby2019} and LoRA~\cite{Hu2022}, and it is also what lets the multi-tenant serving systems of Section~\ref{sec:inference_systems} (Punica~\cite{Chen2023Punica}, S-LoRA~\cite{Sheng2024}, CaraServe~\cite{Li2024CaraServe}) share one resident backbone across many task adapters.
    \item Adapter exchange is included because a P2P network is inherently dynamic: the ability to exchange adapters enables a wider range of compositional combinations and improves overall system functionality. AdapterHub~\cite{Pfeiffer2020hub}, LoraHub~\cite{Huang2023}, and LoraRetriever~\cite{ZhaoLoraRetriever2024} each treat the adapter as a portable, exchangeable artefact by design, independently of one another.
    \item P2P topology is selected as a standalone dimension because operating in such an environment presents coordination challenges that are trivially resolved in centralised systems but remain genuinely open in decentralised ones. Only three corpus systems adopt it: Petals~\cite{Borzunov2023}, the P2P multi-task architecture of {\v{S}}ajina et al.~\cite{Sajina2024}, and gossip learning~\cite{HegeduisGossip2019}.
    \item Discovery is included because it enables dynamic identification of new adapters and their serving across the network without prior knowledge of the peer population. The three systems that engage it at all do so either at the wrong granularity, Petals' DHT routes to transformer-layer shards rather than adapter capabilities~\cite{Borzunov2023}, or by centralising it, as in LoraRetriever's pre-indexed catalogue~\cite{ZhaoLoraRetriever2024} and AdapterHub's browsable repository~\cite{Pfeiffer2020hub}, which is precisely the gap identified in Section~\ref{sec:synthesis_gap:gaps}.
    \item Multi-task fusion is implied by the P2P setting itself: with a larger number of peers contributing specialised adapters, the ability to fuse across tasks becomes a structural necessity rather than an optional feature. Three independently developed mechanisms address this need: attention-based fusion~\cite{Pfeiffer2021fusion}, hub-based composition~\cite{Huang2023}, and mixture-of-experts-style routing among LoRA modules~\cite{LiMixLoRA2024,ZhangMiLoRA2024}.
    \item Privacy is included because adapters frequently encode parameters derived from sensitive or domain-specific data, and isolation guarantees are therefore a relevant architectural concern. It is the least-addressed commitment in the corpus, met only by DP-FedLoRA's formal differential-privacy guarantees for clinical NLP~\cite{XuDPFedLoRA2025} and the federated-LoRA privacy-utility analyses of Liu et al.~\cite{LiuDP2023} and Sun et al.~\cite{SunImprovingLoRA2024}.
    \item The absence of a central coordinator is the defining property of a genuinely peer-to-peer setting: by definition, a P2P network implies no central orchestrator, and any system that retains one falls outside this category. Gossip learning frames eliminating the central aggregator as its own contribution~\cite{HegeduisGossip2019}; Petals relies on volunteer nodes alone~\cite{Borzunov2023}; and {\v{S}}ajina et al.'s architecture likewise requires no coordinating server~\cite{Sajina2024}.
\end{itemize}

Two dimensions were considered and deliberately excluded. Latency was not included as a dimension because it is a continuous evaluation metric rather than a binary architectural commitment; its trade-offs for a P2P setting are acknowledged narratively in the per-pillar sections (Sections~\ref{sec:inference_systems} and~\ref{sec:p2p_federated}) rather than scored in the concept matrix. Incentive mechanisms were also excluded; they are relevant to any future decentralised adapter-sharing design but belong outside the scope of this review. The incentive-mechanism exclusion is discussed further in the limitations in Section~\ref{sec:conclusion:limitations}; latency's exclusion is addressed only in the per-pillar discussion cited above.
...
\paragraph{System selection for the matrix.}
Table~\ref{tab:concept_matrix} scores 19 systems rather than the full 123-record
corpus. The original 17 were selected as follows: the two foundational adapter methods
that every later system builds on (Houlsby adapters, LoRA), plus, for each of
the five pillars, the most architecturally developed published exemplars,
those that make an explicit design commitment on at least one of the seven
dimensions and are either the pillar's canonical reference or its closest
approach to the decentralised target. Two further systems were added
following peer review, both plausible counterexamples to the gap claim
that the original selection omitted: Ryabinin and Gusev's decentralised
Mixture-of-Experts~\cite{RyabininCrowdsourced2020} and
Dec-LoRA~\cite{Ghiasvand2025}; \S\ref{sec:synthesis_gap:gaps} discusses
why each falls short of closing the gap despite scoring among the highest
in the matrix. Incremental variants that share a
predecessor's profile on all seven dimensions are represented by that
predecessor and not listed separately; records that engage none of the seven
dimensions (for example, pure benchmark or survey entries) are omitted. The
matrix is therefore an illustrative cross-section chosen to span the design
space, not an exhaustive scoring, and the gap claim in
Section~\ref{sec:conclusion:contributions} is quantified only over these 17
systems. A full-corpus scoring pass is identified as future work in
...
\centering
\footnotesize
\caption{Concept matrix: 19 selected systems against seven design dimensions of the target
         decentralised adapter-based system. The 17 are an illustrative
         cross-section spanning the five pillars, not an exhaustive scoring of
         the 123-record corpus (selection rule in
         Section~\ref{sec:synthesis_gap:matrix}). \checkmark\ = addressed; (\checkmark) = partially;
...
LoraHub \cite{Huang2023}                    & \checkmark & \checkmark  & $\times$    & $\times$    & \checkmark  & $\times$    & $\times$   \\
...
Gossip Learning \cite{HegeduisGossip2019}   & $\times$   & $\times$   & \checkmark  & $\times$    & $\times$    & $\times$    & \checkmark \\
Decentralised MoE \cite{RyabininCrowdsourced2020} & $\times$ & $\times$ & \checkmark  & \checkmark  & \checkmark  & $\times$    & \checkmark \\
Dec-LoRA \cite{Ghiasvand2025}                & \checkmark & \checkmark  & \checkmark  & $\times$    & $\times$    & (\checkmark)& \checkmark \\
\midrule
\textit{Requirement profile implied by RQ}                 & \checkmark & \checkmark  & \checkmark  & \checkmark  & \checkmark  & $\times^{\dagger}$    & \checkmark \\
\bottomrule
...
The final row of Table~\ref{tab:concept_matrix}, the requirement profile implied by RQ, aggregates the dimensions addressed individually by different bodies of work. It does not represent a proposed system or architecture: this row states the conjunction the review was constructed to test, and is a restatement of the research question, not a finding. None of the 19 systems scored addresses all seven dimensions simultaneously. The combination of these requirements represents a coherent but as yet unexplored research direction, and each unaddressed dimension identifies an open question that future work in this area would need to resolve.

To test whether this result depends on the 19-system selection, all 48 Tier-1 records (the corpus's highest reading-priority tier, \S\ref{sec:methodology:quality:tier1}) were scored against the same seven dimensions and published in full as Supplementary Material S3. Three records (two competing surveys and one software-framework overview, not architectural contributions in their own right) were excluded from scoring; of the remaining 45, the result is graded rather than binary: 1 system satisfies only 1 of the 7 dimensions, 32 satisfy 2, and 12 satisfy 3, with no Tier-1 system reaching 4. Across a much larger, non-curated pool, the frontier is therefore lower, not higher, than the 19-system illustration suggested: $k=3$ is the ceiling, and barely a quarter of the corpus's highest-priority records reach even that. Two clarifications on scoring. First, the marker $-$ denotes a dimension the system does not engage at all (for example, LoRA and the original adapters are single-model training methods that neither provide nor require coordination, so their ``no central coordinator'' cell is $-$ rather than \checkmark); only systems that make an explicit design commitment are marked \checkmark\ or $(\checkmark)$. Second, ``P2P topology'' and ``absence of a central coordinator'' are related but distinct commitments: the former describes how peers connect and exchange modules, the latter whether a coordinating server is required, and the two are deliberately kept separate because a system could in principle adopt a P2P topology while still depending on a central coordinator for orchestration.
...
(Table~\ref{tab:gap_map}) based on their nature: conceptual, algorithmic,
...
\begin{itemize}
  \item \textbf{Adapter discovery protocol.} No reviewed system provides a protocol for a node to discover, across a P2P network, which peers hold adapters relevant to a given query without contacting a central registry. This gap
    ...
    Ryabinin and Gusev's decentralised Mixture-of-Experts~\cite{RyabininCrowdsourced2020}
    (Table~\ref{tab:concept_matrix}) is the closest existing system to this question:
    its DHT resolves already-known expert identifiers to network addresses, while a
    separately, jointly-trained gating network, operating over a fixed set of experts
    known at training time, decides which expert is relevant to a given input. This is
    identifier-level lookup, not capability-level discovery of previously unknown
    adapters, so the gap above survives this counterexample rather than being closed
    by it.
    DHT-based lookup is foregrounded here because of its demonstrated scale and
    bounded $O(\log N)$ lookup cost, but it is one candidate mechanism among
    several: gossip-based propagation, federated indexing of registries,
    replicated registries, and hybrid coordination each offer distinct trade-offs,
    and which of these best sustains capability-level discovery under realistic
    P2P churn remains open (Section~\ref{sec:p2p_federated}).
...
\begin{itemize}
  ...
  \item \textbf{Gossip-based adapter metadata dissemination.} A related open question
    ...
    in $O(\log N)$ rounds~\cite{KarpRumor2000}, is sufficient for adapter
...
\begin{itemize}
  ...
  \item \textbf{Centralised fusion dependency.} AdapterFusion \cite{Pfeiffer2021fusion}
    ...
    P2P setting; training-free algebraic alternatives (Task Arithmetic
    \cite{Ilharco2023TaskArithmetic}, TIES-Merging \cite{Yadav2023TIES}; see
    Section~\ref{sec:adapter_composition:fusion}) remove the training-data dependency
    but assume the adapters to be merged are already co-located and share an
    initialisation lineage. No reviewed method demonstrates fusion of
    independently discovered, independently initialised adapters without a
    shared training lineage or centralised coordination. This raises the open
    architectural question: can probe-based local estimation replace centralised
    fusion training without unacceptable performance loss, and under what
    conditions on probe set size and adapter diversity does the approximation
    degrade?
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Research gap map: identified gaps grouped by quadrant, with the closest
         prior work checked against each gap, unaddressed concept matrix
         dimensions, and open research question. Each gap is a negative claim
         bounded by the 123-paper corpus; the cited works are the nearest
         approaches identified and found not to address the listed dimensions
         (see \S\ref{sec:synthesis_gap:gaps}).}
\label{tab:gap_map}
\setlength{\tabcolsep}{4pt}
\footnotesize
\begin{tabular}{p{1.9cm}p{3.3cm}p{2cm}p{1.9cm}p{3.9cm}}
\toprule
\textbf{Quadrant} &
\textbf{Gap Description} &
\textbf{Closest prior work checked} &
...
Conceptual &
  ...
  P2P context &
  Concept matrix (Table~\ref{tab:concept_matrix}); \cite{Sajina2024,Ponti2023} &
...
\multirow{3}{*}{Algorithmic} &
  ...
  \cite{MaymounkovKademlia2002,ZhaoLoraRetriever2024,Ostapenko2024} &
  Discovery &
  ...
  \cite{Ostapenko2024,Muqeeth2024} &
  Discovery &
  ...
  \cite{ZhaoLoraRetriever2024,Muqeeth2024} &
  Discovery &
...
\multirow{2}{*}{Systems} &
  ...
  \cite{Sheng2024,Chen2023Punica,Li2024CaraServe,Borzunov2023} &
  ...
  \cite{OrmandyGossip2013,HegeduisGossip2019} &
  P2P + No central coord. &
...
\multirow{2}{*}{Empirical} &
  ...
  \cite{ZhangFedPETuning2023,Li2020FedProx,BenDavid2010} &
  Adapter exchange + P2P &
  ...
  &
  No reviewed method demonstrates fusion of independently discovered,
  independently initialised adapters without a shared training lineage or
  centralised coordination &
  \cite{Pfeiffer2021fusion,Ilharco2023TaskArithmetic,Yadav2023TIES} &
  Multi-task fusion &
