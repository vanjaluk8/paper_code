...
carry?} This section surveys the serving systems (Figure~\ref{fig:peft_paradigm}) and makes each assumption
explicit; SQ3 is resolved in the centralised discussion in
Section~\ref{sec:synthesis_gap:answers}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/fig_concept_peft_paradigm.pdf}
  \caption{The PEFT serving paradigm, viewed through the adapter-paradigm
    figure introduced in
    Section~\ref{sec:background}: a single frozen backbone is shared across
    concurrent requests, each associated with a lightweight task-specific
    adapter. Serving systems must manage a population of adapters in GPU
    memory while minimising per-request adapter swap latency~\cite{Sheng2024}.}
  \label{fig:peft_paradigm}
\end{figure}
...
improvement in average request serving latency of up to 1.7$\times$ and up to 99\% SLO
...
hierarchy to include SSD and object storage. Chen et al.~\cite{ChenFlashServe2025} proposed
...
\noindent The serving findings of this section are consolidated, alongside the
