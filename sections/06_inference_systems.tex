...
\noindent\textbf{SQ3:} \textit{What serving and memory-management mechanisms
exist for multi-adapter inference, and what centralisation assumptions do they
carry?} This section surveys the serving systems (Figure~\ref{fig:concept_peft_paradigm}) and makes each assumption
explicit; SQ3 is resolved in the centralised discussion in
...
inference computation itself for short requests~\cite{Sheng2024}. The multi-tenant adapter serving
...
Yu et al.~\cite{YuOrca2022} introduced iteration-level scheduling with selective batching, which dynamically
...
for single-model generation. Kwon et al.~\cite{KwonPagedAttention2023} introduced PagedAttention,
...
Chen et al.~\cite{Chen2023Punica} introduced Punica, among the first systems explicitly designed for
...
Sheng et al.~\cite{Sheng2024} addressed the GPU capacity constraint with S-LoRA, a system that
...
fine-grained paged allocation for both simultaneously, and a \textit{tensor parallelism
strategy paired with custom CUDA kernels} that batch LoRA computation across adapters
of heterogeneous rank within the same request batch. S-LoRA
...
characteristics \cite{Sheng2024}.
...
Li et al.~\cite{Li2024CaraServe} proposed CaraServe (released as an arXiv preprint and
subsequently published, under the name Toppings, at USENIX ATC 2025), which augments
S-LoRA by offloading LoRA computations to CPU cores rather than merely using the CPU as
slow memory. Because LoRA adapter matrices are small (a rank-4 adapter for a hidden
dimension 4096 occupies only $2 \times 4 \times 4096 \times 4 \approx 256$~KB per
layer), their matrix products can be computed efficiently on CPU while the GPU
processes the base model forward pass. CaraServe early-starts adapter prefilling on CPU
while adapter weights are concurrently transferred to GPU, switching to GPU execution
once loading completes; under continuous batching, loading a newly requested adapter to
GPU would otherwise interrupt the ongoing decoding of all inflight queries, which
CaraServe avoids via a pipeline loading scheme that simultaneously coordinates CPU
prefilling, GPU transfer, and ongoing decoding. The published version reports an
improvement in average request serving latency of up to 1.7$\times$ and up to 99\%
service-level objective (SLO) attainment. The rank-aware placement policy: high-rank adapters remain on GPU,
...
FlexGen~\cite{ShengFlexGen2023} and AlpaServe~\cite{LiAlpaServe2023} address related single-provider resource-constraint problems: FlexGen offloads generation across GPU, CPU, and disk to run large models on a single GPU, while AlpaServe exploits statistical multiplexing across model-parallel replicas to serve bursty multi-model workloads, neither addressing adapter populations or cross-peer coordination.

Table~\ref{tab:serving_comparison} summarises the four multi-tenant LoRA serving
systems examined above (Punica, S-LoRA, CaraServe, and the edge-oriented
EdgeLoRA) along the axes a decentralised deployment would stress.

\begin{table}[H]
\centering
\caption{Multi-tenant LoRA serving systems compared on the axes a decentralised
setting would stress. All four assume a single operator with centralised
scheduling and co-located compute; none addresses peer discovery, variable
inter-node bandwidth, or node churn. Figures are as reported by each system's
cited primary against its own stated baseline and are not re-normalised across
rows.}
\label{tab:serving_comparison}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l p{2.5cm} p{3.0cm} p{2.0cm} p{3.0cm}}
\toprule
\textbf{System} & \textbf{Adapter\newline residence} &
\textbf{Multi-adapter\newline batching} & \textbf{Tier beyond\newline GPU HBM} &
\textbf{Headline result\newline (vs.\ own baseline)} \\
\midrule
Punica~\cite{Chen2023Punica} & All active adapters resident in GPU HBM;
asynchronous on-demand load & SGMV (segmented gather matrix--vector) kernel;
one shared base-model batch & None (HBM only) & $12\times$ throughput vs.\
state-of-the-art LLM serving \\
S-LoRA~\cite{Sheng2024} & Paged in a unified GPU/CPU pool; swapped per request &
Unified Paging with heterogeneous batched kernels; mixed adapter ranks in one
batch & CPU DRAM (Unified Paging) & up to $4\times$ throughput; thousands of
adapters on one GPU \\
CaraServe~\cite{Li2024CaraServe} & Rank-aware split: high-rank on GPU, low-rank
computed on CPU cores & CPU-side prefill early-start, pipelined GPU transfer
during ongoing decode & CPU compute $+$ DRAM & up to $1.7\times$ lower serving
latency; up to 99\% SLO attainment \\
EdgeLoRA~\cite{Shen2025EdgeLoRA} & Compressed (quantised / pruned) adapters
time-sharing device memory & Dynamic time-sharing of GPU memory across tenants &
Device DRAM / flash & up to 70\% memory saving, within 2\% accuracy (edge
devices) \\
\bottomrule
\end{tabular}
\end{table}
...
hierarchy to include SSD and object storage. Chen~\cite{ChenFlashServe2025} proposed
...
historical request patterns to minimise cache miss latency. Fu et al.~\cite{FuServerlessLLM2024}
...
Ni et al.~\cite{NiPredictive2025} introduced a predictive adapter prefetching system that models
...
Shen et al.~\cite{Shen2025EdgeLoRA} proposed EdgeLoRA, a multi-tenant LoRA serving system for edge
...
Cai et al.~\cite{CaiEdgeLLM2024} introduced a collaborative edge--cloud LLM inference framework
...
accuracy-critical queries. Zhang et al.~\cite{ZhangEdgeShard2025} extended edge deployment to
...
Wang et al.~\cite{WangTailorLLM2026} (post-review top-up find,
\S\ref{sec:methodology:search}) proposed TailorLLM, an end--cloud collaborative
system distinguished by two mechanisms neither prior system uses: a
cold/hot-module decomposition of the pre-trained parameters that cuts
adapter-transmission overhead, and an imitation-learning-based manager that
decides which LoRA modules a device keeps resident, rather than a fixed or
LRU policy. Reported reductions are up to 69.8\% cloud-resource consumption
and 62\% inference latency against their baselines.

\subsection{Distributed and Collaborative Inference}
\label{sec:inference_systems:distributed}

Borzunov et al.~\cite{Borzunov2023} introduced Petals, a collaborative inference system for large
...
Hu et al.~\cite{HuDistributed2022} addressed the bandwidth cost of split inference, proposing
...
Wu et al.~\cite{WuRock2025} proposed Rock, a multi-adapter serving system for multimodal LLMs
that coordinates thousands of domain-specific adapters across a GPU cluster using a
centralised orchestration layer. Zhu et al.~\cite{ZhuCannikin2025} introduced Cannikin, which
eliminates straggler effects in multi-LoRA batched inference by decoupling the prefill
and decode phases and routing them to separate GPU resources. Shi et al.~\cite{Shi2025AuLoRA}
...
Xu et al.~\cite{XuDyMergeLoRA2026} (post-review top-up find,
\S\ref{sec:methodology:search}) proposed DyMerge-LoRA, which targets a case
none of Table~\ref{tab:serving_comparison}'s four systems address: requests
composed of \emph{multiple} LoRA adapters rather than a one-to-one
request-adapter mapping. It dynamically fuses the requested adapters
on-GPU via a dedicated matrix-fusion kernel, reporting up to $4\times$
throughput over prior multi-tenant methods on composite-adapter workloads.
