...
| Blocking 4 (add Dec-LoRA/MoE rows) | done | `sections/09_synthesis_gap.tex` Table 10 — both added, k=4 each (tied highest in the matrix); §9.4's discovery-gap paragraph now defends the identifier-vs-capability distinction against Ryabinin's mechanism directly, using the primary-source quotes already verified in `OPEN_QUESTIONS.md` S-2 | |
| Blocking 5 (appraisal instrument) | disputed | author declined: §3.4's existing justification (classical RoB instruments don't transfer to a CS methods/systems corpus) is judged PRISMA-compliant and sufficient on its own; the reviewer's specific two-dimension instrument (baseline adequacy + threats-reporting, scored per-paper) is not a standardised SLR instrument and the added rigor was judged not worth the effort for this revision | |
...
| Major 17 (inclusion-rule / I-E table) | done | `sections/12_appendix.tex` Table A2 (Appendix A.3); `sections/03_methodology.tex` §3.2 now formalises the corpus's existing core/background stratification (89/34, 72.4%/27.6%) in the main text rather than tightening the admission rule itself, per the review's own suggested fix | |
...
| Major 20 (citation-verification pass) | done | DOI resolution: `OPEN_QUESTIONS.md` Major 21 — 125/128 resolve, 3 fixed. Author/venue/claim verification: done for every reference the reviewer's own PASS 5 spot-check flagged (E-1 through E-15, all now fixed); a blind sweep of the remaining ~120 unflagged references was considered and declined (author decision — no specific signal of further problems) | |
| Major 21 (§9.3 dimension provenance) | done | `sections/09_synthesis_gap.tex` §9.3 — each of the 7 dimension-justification bullets now cites 2-3 corpus papers as evidence, from `verify/major21_dimension_provenance_scaffold.md`; no new bibliography entries needed | |
