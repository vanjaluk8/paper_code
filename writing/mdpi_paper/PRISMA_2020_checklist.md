...
| **Availability of data, code, and other materials** | 27 | Report which of the following are publicly available and where they can be found: template data collection forms; data extracted from included studies; data used for all analyses; analytic code; any other materials used in the review. | Held in two private repositories (see Data Availability statement): `papers_code` (this manuscript, curated corpus `papers_repo/G0–G6`, PRISMA package) and `slr_engine` (snowballing/retrieval/screening/extraction pipeline source, incl. `app/visualise.py`, `app/prisma.py`). Version-controlled deliverables: `13_final_reading_list_2026-05-12.csv`, `PRISMA_summary_2026-04-21.md`. Intermediate pipeline outputs (`pipeline_unified.csv`, retrieval/screening logs, per-record extraction spreadsheet) are regenerable local artefacts, not separately version-controlled; available on request. Both repositories are private; public release deferred until after thesis examination. |
