...
The arXiv inclusion threshold serves as a minimal proxy for community
engagement: a preprint cited by at least one peer-reviewed work in the corpus
has demonstrably influenced the reviewed literature. The threshold was set
conservatively low to favour recall, as recent preprints (2025--2026) may not
yet have accumulated peer-reviewed citations. The implications of this
criterion for corpus composition are discussed in
...
\textbf{Layer~1} excluded records published before 2021 based on publication
...
$731{+}60{=}791$ EXCLUDE. The 19 UNCERTAIN records are carried forward and resolved during the
subsequent abstract-level review (Section~A.7 in this appendix).
...
metadata via the Semantic Scholar batch API and OpenAlex. A
...
\texttt{contribution\_codes} & Novel-contribution tag(s) & \texttt{\textstar A1/S1/R1/R2/T2/M2} combinations & 22/224 \\
...
\texttt{notes\_raw} & Full-text reading notes (methods, results, \texttt{\S} and \texttt{\textstar} tags) & free text & 190/224 \\
...
The final corpus of 123 papers spans the period 2002--2026, with a marked
...
review across all 552 records (464 enriched corpus records after rescues were
resolved, plus 88 records from forward citation snowballing on Scopus and WoS
that were title-screened and added to the review). To support decision
...
records reviewed; the intermediate second pass carried 173 DEFER records; all
were subsequently resolved to SKIP in the final classification and none
advanced to data extraction).}
\label{tab:abstract}
\begin{tabular}{lrr}
\toprule
...
(\texttt{08\_zotero\_ready\_2026-05-02.ris}). The full-text review queue
(\texttt{09\_fulltext\_review\_queue\_2026-05-02.csv}) contains \textbf{387}
records; of these, \textbf{190} underwent data extraction, and a manual
Scopus/WoS top-up brought the extraction set to \textbf{224}
(\texttt{11\_data\_extraction\_2026-05-12.csv}). Every record and its
per-stage membership are traceable in the unified pipeline file
(\texttt{pipeline\_unified.csv}). The final reading list
...
access is detailed in the Data Availability statement.
...
of \v{S}ajina~\cite{Sajina2021} lacks a stable bibliographic identifier
suitable for API-based citation retrieval, and \v{S}ajina et
