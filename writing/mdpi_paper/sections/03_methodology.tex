...
\mentornote{OVAJ DIO TREBA POGLEDATI I ODLUČITI KAKO PRISTUPITI:
This review currently reports no inter-rater reliability statistic for
screening, on the grounds that a single reviewer conducted all title, abstract, and
full-text decisions and no independent second human reviewer was available
(consistent with PRISMA 2020 Item~8, which does not mandate dual screening). An
external audit of this manuscript flagged this as the weakest point of the
methodology section and suggested two possible remedies, neither of which has
been implemented yet: \textbf{(A)} recruit a colleague or co-supervisor to
independently re-screen a random 5--10\% sample of the 972 title-screened
records, blind to the original decisions, and report percent agreement; or
\textbf{(B)} the author independently re-screens the same random sample after a
time delay, blind to their own prior labels, and reports intra-rater agreement
as a documented (weaker, but single-author-feasible) substitute. Open questions
for discussion: (1) Is a formal spot-check expected or optional for a
single-author doctoral SLR at this Faculty, given PRISMA 2020's Item~8 wording
does not mandate dual screening? (2) If a spot-check is advisable, is Option A
or Option B more appropriate given time constraints before submission? (3) If
neither is feasible before the submission deadline, is the existing disclosure
(single-reviewer limitation, cited 5--13\% miss-rate estimate, Threats to
Validity \mbox{\S\ref{sec:conclusion:threats}}) sufficient as reported, or does it need
strengthening even without a new measurement? This note is a working flag for
mentor discussion and must be deleted --- along with its result, once
resolved, folded into the permanent Internal Validity paragraph in
\mbox{\S\ref{sec:conclusion:threats}} --- before the manuscript is submitted.}
...
Figure~\ref{fig:prisma}), and the remaining \textbf{53} second-pass-included records (43.1\%) were
screened in a second abstract-review pass (\texttt{abstract-2nd-pass}) before
data extraction.
...
\setlength{\tabcolsep}{5pt}
  ...
  Accuracy & $\approx$ FL quality (20--50\% slower) & Unknown \\
  \midrule
...
\textbf{26} (\textbf{79\%}) release usable code or artifacts. FedAvg is
available only through a maintained downstream framework (TensorFlow
Federated) rather than its original release. For \textbf{6} systems
(\textbf{18\%})---Expert Choice, MT-EF ({\v{S}}ajina et al.), Ostapenko et al.,
DP-FedLoRA, SLoRA, and Gossip Learning---no public artifact could be located at
the check date. The gap is concentrated among the systems closest to this
review's object of study: none of the three federated or P2P fine-tuning
methods (SLoRA, DP-FedLoRA, Gossip Learning), the MT-EF P2P architecture, nor
the modular-LoRA composition framework (Ostapenko et al.) releases a locatable
artifact, despite several being recent and otherwise well-documented.
