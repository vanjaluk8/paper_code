...
scores are provided as accompanying material in
\texttt{figures/quality\_appraisal\_scored.csv} (123 records; each scored
0--2 on Q1--Q6 with a total and quality band). The low-scoring records are
...
release only partial artifacts, so most of the core evidence body cannot be
re-run from released code. Baseline comparison is the strongest of the three
dimensions. \textbf{40} (83.3\%) evaluate against a named baseline, and a
further \textbf{6} (12.5\%) do so only partially. Most striking is the dearth
of critical self-assessment. Only \textbf{11} (22.9\%) report an explicit
...
\texttt{threats\_reported}). These three dimensions remain outside the 0--12
rubric by design. They are reported here as a separate, clearly-labelled
appraisal rather than folded into the aggregate score.
