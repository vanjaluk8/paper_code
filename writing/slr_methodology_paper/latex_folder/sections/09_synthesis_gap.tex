...
\midrule
Houlsby adapters \citep{Houlsby2019}        & \checkmark & $\times$    & $\times$    & $\times$    & $\times$    & $\times$    & \checkmark \\
LoRA \citep{Hu2022}                          & \checkmark & $\times$    & $\times$    & $\times$    & $\times$    & $\times$    & \checkmark \\
AdapterHub \citep{Pfeiffer2020hub}           & \checkmark & \checkmark  & $\times$    & (\checkmark)& $\times$    & $\times$    & $\times$   \\
AdapterFusion \citep{Pfeiffer2021fusion}     & \checkmark & (\checkmark)& $\times$    & $\times$    & \checkmark  & $\times$    & $\times$   \\
LoraHub \citep{Huang2023}                    & \checkmark & \checkmark  & $\times$    & $\times$    & \checkmark  & $\times$    & (\checkmark)\\
LoraRetriever \citep{ZhaoLoraRetriever2024} & \checkmark & \checkmark  & $\times$    & (\checkmark)& \checkmark  & $\times$    & $\times$   \\
S-LoRA \citep{Sheng2024}                     & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & $\times$    & $\times$   \\
Punica \citep{Chen2023Punica}                & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & $\times$    & $\times$   \\
CaraServe \citep{Li2024CaraServe}            & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & $\times$    & $\times$   \\
Petals \citep{Borzunov2023}                  & \checkmark & $\times$    & \checkmark  & \checkmark  & $\times$    & $\times$    & \checkmark \\
MT-EF / \v{S}ajina \citep{Sajina2024}        & \checkmark & (\checkmark)& \checkmark  & $\times$    & \checkmark  & $\times$    & \checkmark \\
FedPETuning \citep{ZhangFedPETuning2023}     & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & (\checkmark)& $\times$   \\
DP-FedLoRA \citep{XuDPFedLoRA2025}           & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & \checkmark  & $\times$   \\
FLoRA \citep{WangFLoRA2024}                  & \checkmark & \checkmark  & $\times$    & $\times$    & $\times$    & $\times$    & $\times$   \\
MixLoRA \citep{LiMixLoRA2024}                & \checkmark & \checkmark  & $\times$    & $\times$    & \checkmark  & $\times$    & $\times$   \\
MiLoRA \citep{ZhangMiLoRA2024}               & \checkmark & \checkmark  & $\times$    & $\times$    & \checkmark  & $\times$    & $\times$   \\
Gossip Learning \citep{HegeduisGossip2019}   & (\checkmark)& $\times$   & \checkmark  & $\times$    & $\times$    & $\times$    & \checkmark \\
\midrule
...
\begin{itemize}
  \item \textbf{Adapter discovery protocol.} No reviewed system provides a protocol for a node to discover, across a P2P network, which peers hold adapters relevant to a given query without contacting a central registry. This gap
    ...
    proven at scale in systems such as Kademlia \citep{MaymounkovKademlia2002},
...
\begin{itemize}
  ...
  \item \textbf{Gossip-based adapter metadata dissemination.} A related open question
    ...
    in $O(\log N)$ rounds \citep{OrmandyGossip2013}, is sufficient for adapter
...
\begin{itemize}
  \item \textbf{Adapter reuse quality under non-IID distributions.} The federated PEFT literature has demonstrated that adapter aggregation degrades under heterogeneous data distributions \citep{ZhangFedPETuning2023, Li2020FedProx}, but no reviewed work has derived bounds on adapter reuse quality as a function of distribution distance between the contributing peer's training distribution and the requesting peer's query distribution. The domain adaptation literature establishes that such a bound is theoretically tractable (\citet{BenDavid2010} ground similar degradation guarantees in divergence measures between source and target distributions), but this result has not been instantiated for the adapter reuse setting, where the source distribution is unknown to the receiving peer and adapter rank introduces an additional capacity constraint. Formally characterising this degradation as a function of distribution distance and adapter rank remains an open theoretical question.
  
  \item \textbf{Centralised fusion dependency.} AdapterFusion \citep{Pfeiffer2021fusion}
...
and the open research question each gap defines. This map constitutes
the primary analytical contribution of the SLR: a structured account of
what the literature has not yet addressed and what a future research programme
...
\midrule
Conceptual &
  No prior work frames adapters as autonomous, transferable knowledge units
  with identity, provenance, and capability in a P2P context &
  G0--G6 &
...
inference is a coherent, theoretically motivated by converging evidences from adjacent fields, and empirically unexplored
research direction. The evidence base assembled in this review indicates that
