...
The transformer \citep{Vaswani2017} is a sequence-to-sequence architecture based
...
transformer backbones (GPT \citep{RadfordGPT22019,BrownGPT32020}, BERT, T5 \citep{RaffelT52020}, LLaMA \citep{TouvronLLaMA2023}) capture rich linguistic and factual
...
can be adapted to downstream tasks with minimal additional training. \citet{Devlin2019}
...
\begin{enumerate}
  \item \textbf{Bottleneck adapters} \citep{Houlsby2019}: compact modules inserted
  ...
  \item \textbf{Low-Rank Adaptation (LoRA)} \citep{Hu2022}: low-rank matrix products
    $\Delta W = BA$ added in parallel to the frozen attention weight matrices
    (Equation~\ref{eq:lora}).
  \item \textbf{Prefix tuning} \citep{LiLiang2021}: learnable continuous vectors
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{fig_concept_peft_paradigm.pdf}
...
\citep{Pfeiffer2020hub}.}
  \label{fig:concept_peft_paradigm}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{fig_peft_adapter_types.pdf}
  \caption{Adapter structural types: (a) bottleneck adapter modules inserted into a transformer block; (b) LoRA low-rank matrices added in parallel to attention weights; (c) prefix tuning prepends learnable key-value vectors.}
...
decentralised lookup in P2P systems. \citet{MaymounkovKademlia2002} introduced
...
probability \citep{OrmandyGossip2013}. Gossip protocols are inherently fault-tolerant
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{fig_p2p_dht_lookup.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{fig_p2p_gossip.pdf}
...
%\begin{figure}[H]
%  \centering
%  \includegraphics[width=0.65\linewidth]{fig_concept_marketplace.png}
